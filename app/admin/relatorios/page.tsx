...
export default function RelatoriosPage() {
  ...
  const loadColaboradores = async () => {
    try {
      ...
      // Carregar perfil do usuário para verificar permissões
      const profileResponse = await fetch('/api/relatorios?verificarPermissao=true');
      const profileResult = await profileResponse.json();
      if (profileResult._userProfile) {
        setUserProfile(profileResult._userProfile);
        // Se não for admin, travar filtro de colaborador
        if (!profileResult._userProfile.isAdmin && profileResult._userProfile.colaboradorId) {
          setSelectedColaborador(profileResult._userProfile.colaboradorId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FDECFB] via-[#E7D3FF] to-white">
      ...
      <div className="container mx-auto px-4 md:px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => !filtroTravado && setSelectedColaborador(e.target.value === 'todos' ? 'todos' : parseInt(e.target.value))}
                disabled={filtroTravado}
                className={`w-full px-4 py-2 border border-purple-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-400 bg-white text-sm ${filtroTravado ? 'cursor-not-allowed opacity-60' : ''}`}
              >
                {!filtroTravado && <option value="todos">Todas</option>}
                {colaboradores.map((c) => (
          ...
          <div className="mt-4 pt-4 border-t border-purple-100">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm text-gray-600">Projeção futura (estimativa)</span>
              </label>
            </div>
        ...
        {/* Blocos de Realizado vs Projeção */}
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-200">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-xl text-green-700">R$ {(totaisExtras?.faturamento || stats.total).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Comissão Colaboradores:</span>
                <span className="font-bold text-pink-600">R$ {(totaisExtras?.comissaoRealizada || stats.comissaoColaboradores).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center pt-3 border-t border-green-200">
                <span className="text-gray-700 font-medium">Faturamento Líquido:</span>
                <span className="font-bold text-xl text-green-800">R$ {(totaisExtras?.faturamentoLiquido || stats.comissaoSalao).toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-sm text-gray-600">
                <span>Ticket Médio:</span>
                <span className="font-medium">R$ {stats.ticketMedio.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center text-sm text-gray-600">
                <span>Total de Atendimentos:</span>
                <span className="font-medium">{stats.totalAtendimentos}</span>
              </div>
            </div>
          </div>

          {/* Bloco Projeção */}
          {incluirProjecao && totaisExtras && (
            <div className="bg-gradient-to-br from-amber-50 to-yellow-50 rounded-2xl p-6 border border-amber-200 relative">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Projeção Faturamento:</span>
                  <span className="font-bold text-xl text-amber-700">R$ {(totaisExtras.projecaoFaturamento || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Comissão Estimada:</span>
                  <span className="font-bold text-violet-600">R$ {(totaisExtras.projecaoComissao || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pt-3 border-t border-amber-200">
                  <span className="text-gray-700 font-medium">Líquido Estimado:</span>
                  <span className="font-bold text-xl text-amber-800">R$ {(totaisExtras.projecaoLiquido || 0).toFixed(2)}</span>
                </div>
              </div>
              <p className="mt-4 text-xs text-amber-600 flex items-center gap-1">
                ...
                Baseado em agendamentos futuros. Comissão só é gerada após realização.
              </p>
            </div>
          )}

          {/* Placeholder quando projeção não está ativa */}
          {!incluirProjecao && (
            <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200 flex flex-col items-center justify-center text-center">
              <svg className="w-12 h-12 text-gray-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
              </svg>
              <p className="text-gray-500 text-sm">Ative &quot;Projeção futura&quot; nos filtros para ver estimativas</p>
            </div>
          )}
        </div>
