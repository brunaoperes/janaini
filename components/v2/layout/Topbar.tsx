...
export default function Topbar({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: ReactNode }) {
  ...
  return (
    <header style={{
      ...
      padding: '22px clamp(18px,3vw,34px)', borderBottom: '1px solid var(--nb-rule)',
      ...
      <div style={{ minWidth: 0 }}>
        <h1 className="nb-display" style={{ margin: 0, fontSize: 26, lineHeight: 1.1, color: 'var(--nb-ink)' }}>{title}</h1>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingLeft: 12, borderLeft: '1px solid var(--nb-rule)' }}>
          ...
          <Icon name="ChevronDown" size={16} className="nb-ink-faint" />
        </div>
      </div>
    </header>
  );
